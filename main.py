...
class NiftyOptionsAlgo:
    ...
    def _place_order(self, side, token, symbol, price, quantity):
        """
        BUY  → Limit order at ltp + BUY_LIMIT_BUFFER (3 pts)
               Controls entry price. Fills instantly at liquid ITM strikes.
               If missed → no fill → no trade → algo retries on next signal.

        SELL → Market order always.
               Exit MUST execute — SL/target must not be delayed by limit.
               MKT on 200pt ITM Nifty options fills in <2s with ~0.5pt slippage.
        """
        is_buy = (side == 'BUY')

        if config.PAPER_TRADE:
            oid = f"PAPER_{side}_{datetime.datetime.now().strftime('%H%M%S')}"
            # Simulate: buy fills at ltp+3, sell fills at ltp (MKT)
            sim_price = round(float(price) + config.BUY_LIMIT_BUFFER, 2) if is_buy else float(price)
        ...
        try:
            ...
            logger.info(f"LIVE {side} {symbol} ref={price:.2f} "
                        ...
                        f"qty={quantity} amo={amo} | id={oid} | resp={resp}")
